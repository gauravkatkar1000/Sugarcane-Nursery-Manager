...
function isActive(w) {
    return w.active === true || w.active === 'true' || w.active === 1 || w.active === 'TRUE'
}

function isPresent(r) {
    return r.present === true || r.present === 'true' || r.present === 1 || r.present === 'TRUE'
}
...
export default function Labour() {
    ...
    const saveAttendanceFn = useAppStore((s) => s.saveAttendance)
    ...
    // Pre-populate present toggles from fetched attendance
    useEffect(() => {
        if (tab !== 'attendance') return
        const map = {}
        activeWorkers.forEach((w) => { map[w.id] = false })
        attendance.forEach((r) => {
            if (r.date === date) map[r.worker_id] = isPresent(r)
        })
        setPresent(map)
        setSaved(false)
    }, [attendance, activeWorkers, date, tab])

    // Load summary when month or tab changes
    useEffect(() => {
        if (tab !== 'summary') return
        const { first, last } = monthRange(month)
        setLoadingSummary(true)
        fetchAttendance(first, last)
    ...
    const dailyTotal = useMemo(
        () => activeWorkers.reduce((sum, w) => present[w.id] ? sum + (Number(w.daily_rate) || 0) : sum, 0),
        [present, activeWorkers]
    )

    const wageSummary = useMemo(() => {
        return activeWorkers.map((w) => {
            const days = summaryData.filter((r) => r.worker_id === w.id && isPresent(r)).length
            return {
                id: w.id,
                name: w.name,
                daily_rate: Number(w.daily_rate) || 0,
                days,
                total: days * (Number(w.daily_rate) || 0),
            }
        })
    }, [activeWorkers, summaryData])
    ...
    async function handleSaveAttendance() {
        ...
        const ok = await saveAttendanceFn(date, records)
        if (ok) setSaved(true)
    }
    ...
    async function handleAddWorker(e) {
        ...
        if (ok) {
            setAddModal(false)
            setWorkerForm({ name: '', daily_rate: '', phone: '' })
        }
    }
    ...
    return (
        <div className="space-y-4">
                            ...
                            {activeWorkers.map((w) => (
                                <div key={w.id} className="flex items-center justify-between py-3 gap-3">
                                    ...
                                    <div className="flex gap-2 shrink-0">
                                        <button
                                            onClick={() => setPresent((p) => ({ ...p, [w.id]: true }))}
                                        ...
                                        <button
                                            onClick={() => setPresent((p) => ({ ...p, [w.id]: false }))}
                                            className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${
                                                present[w.id] === false && present[w.id] !== undefined
                    ...
                    {activeWorkers.length > 0 && (
                        <div className="flex items-center gap-3 pt-1">
                            <button
                                ...
                                disabled={loading.saveAttendance}
                                    ...
                                    {wageSummary.length === 0 ? (
                                        ...
                                        wageSummary.map((r) => (
                                            <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="table-td pr-4 font-semibold text-green-700">
                                ...
                                {wageSummary.length > 0 && (
                                    <tfoot>
                                        <tr className="border-t-2 border-gray-100 bg-gray-50">
                                            ...
                                            <td className="table-td pr-4 font-bold text-green-700 text-lg">
            ...
            <Modal
                open={addModal}
                onClose={() => { setAddModal(false); setWorkerForm({ name: '', daily_rate: '', phone: '' }) }}
                title="Add Worker"
                maxWidth="max-w-sm"
            >
            ...
            >
                <p className="text-sm text-gray-600 mb-6">
                    ...
                    They will no longer appear in attendance. Existing records are preserved.
                </p>
                <div className="flex gap-3">
                    ...
                    <button
                        ...
                        disabled={loading[`deactivate_${deactivateTarget?.id}`]}
